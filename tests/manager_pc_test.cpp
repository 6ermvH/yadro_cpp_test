...
TEST(ManagerPCBase, Release) {
  ManagerPC pc(2, 10);
  pc.assign(0, 100, Time("10:00"));
  EXPECT_NO_THROW(pc.release(100, Time("11:00")));
...
TEST(ManagerPCBase, Stats) {
  ManagerPC pc(1, 20);
  pc.assign(0, 123, Time("09:00"));
  pc.release(123, Time("11:00"));
...
TEST(ManagerPCBase, InvalidAssign) {
  ManagerPC pc(1, 10);
  EXPECT_THROW(pc.assign(5, 999, Time("10:00")), std::runtime_error);
}
